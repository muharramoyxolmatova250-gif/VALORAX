  ...
  return (
    ...
    >
      <div className="relative h-40">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#121417] via-black/10 to-transparent" />
        <div className="absolute top-2 left-2 right-2 flex items-start justify-between">
          ...
          {lot.verified && (
            <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-sm"
              ...
              <Shield size={8} style={{ color: GOLD }} />
              <span className="text-[8px] font-bold tracking-wider" style={{ color: GOLD }}>VERIFIED</span>
            </div>
          )}
        </div>
        <div className="absolute bottom-2 left-3">
          <div className="flex items-center gap-1.5 mb-0.5">
            <motion.div animate={{ opacity: [1, 0.2, 1] }} transition={{ repeat: Infinity, duration: 1.6 }}
              className="w-1.5 h-1.5 rounded-full bg-red-500" style={{ boxShadow: '0 0 5px rgba(239,68,68,0.9)' }} />
            <span className="text-[8px] tracking-widest font-bold text-white/40 uppercase">LIVE</span>
          </div>
          <p className="text-white font-black text-sm leading-tight">{lot.title}</p>
        </div>
      </div>

      <div className="p-3 space-y-2.5">
        <p className="text-[9px] text-white/35 leading-snug">{lot.subtitle}</p>
        ...
        <div className="grid grid-cols-3 gap-1.5">
          <div className="p-2 rounded-sm" style={{ background: 'rgba(212,175,55,0.04)', border: `1px solid ${GOLD_FAINT}` }}>
            ...
            <p className="font-black text-[11px] mt-0.5" style={{ color: GOLD }}>
  ...
  return (
    <div className="space-y-4">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {filtered.map((lot, i) => (
            <motion.div key={lot.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
