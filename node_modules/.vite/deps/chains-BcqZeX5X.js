import { A as BaseError, C as isHex, D as RlpDepthLimitExceededError, E as InvalidHexValueError, O as RlpListBoundaryExceededError, S as size, _ as pad, a as hexToBigInt, d as toBytes$1, g as toHex$1, k as RlpTrailingBytesError, l as hexToBytes, m as numberToHex$1, p as bytesToHex, s as hexToNumber$1, v as padHex, x as trim } from "./stringify-FRpwCepn.js";
import { $ as stringify, $t as InvalidYParityError, B as from$8, C as from$7, Ct as getCallError, D as fromString$1, E as fromNumber, F as toNumber, H as fromString, Ht as parseAccount, I as toString, Jt as InvalidLegacyVError, Kt as InvalidChainIdError$1, L as trimLeft, M as size$1, N as slice, O as isEqual$1, P as toBigInt, Q as validate, Qt as InvalidStorageKeySizeError$1, R as validate$1, S as concat$1, St as extract, T as fromBytes, U as isEqual, V as fromHex$1, W as padLeft, Xt as InvalidSerializedTransactionError, Zt as InvalidSerializedTransactionTypeError, b as InvalidLengthError, bt as assertRequest, dn as defineTransactionRequest, et as BaseError$1, fn as formatTransactionRequest$1, i as isAddressEqual, j as random, k as padLeft$1, ln as maxUint16, sn as createCursor, un as maxUint256, x as assert$3, xt as serializeStateOverride, z as concat } from "./call-BXLbhHje.js";
import { $ as isEqual$2, $t as formatTransactionReceipt$1, B as fromRpc$7, D as toRpcList$1, E as fromRpcList$1, G as yParityToV, Gt as InvalidVersionedHashSizeError, H as toHex$3, Ht as getTransactionType, I as assert$4, J as Field, Jt as sha256, K as secp256k1, Kt as InvalidVersionedHashVersionError, L as extract$1, Q as fromPublicKey, Qt as defineTransactionReceipt, R as from$11, S as getAction, T as getCode, U as toRpc$6, Ut as toBlobSidecars, V as fromTuple$3, W as toTuple$3, Wt as EmptyBlobError, X as checksum, Xt as blobsToCommitments, Y as assert$5, Yt as blobsToProofs, Z as from$10, Zt as toRlp, an as formatTransaction$1, at as create, ct as validate$3, dt as sha512, et as validate$2, g as parseEventLogs, in as defineTransaction, it as toHex$2, j as getSelector, lt as sha256$1, n as sendTransaction, nn as defineBlock, nt as fromHex$3, ot as keccak256, q as createCurve, qt as commitmentsToVersionedHashes, rt as toBytes$2, st as sha256$2, t as writeContract, tn as formatLog, tt as from$9, ut as sha384, v as verifyHash$1, w as getTransaction, y as readContract, z as fromHex$2, zt as AccountNotFoundError } from "./writeContract-DDxeKU4X.js";
import { B as isAddress, K as concatHex, L as slice$1, U as keccak256$1, W as InvalidAddressError, t as encodeFunctionData, z as sliceHex } from "./encodeFunctionData-CpXvefvC.js";
import { S as TipAboveFeeCapError$1, m as FeeCapTooHighError$1 } from "./createBatchScheduler-DtXytP1y.js";
...
function verify$2(options) {
	...
	if (rpId !== void 0) {
		...
		const expectedRpIdHash = sha256$2(fromString$1(rpId), { as: "Bytes" });
	...
	const clientDataJSONHash = sha256$2(fromString(clientDataJSON), { as: "Bytes" });
	return verify$4({
		hash: true,
...
function fromCreate2(options) {
	...
	return from$10(slice(keccak256(concat(fromHex$1("0xff"), from, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function assert$2(config) {
	...
	if (typeof salt !== "undefined" && size$1(salt) !== 32) throw new InvalidConfigError({ reason: "salt must be 32 bytes" });
	...
	for (const owner of owners) {
		if (!validate$2(owner.owner) || toBigInt(owner.owner) === 0n) throw new InvalidConfigError({ reason: "owner cannot be zero" });
...
function getAddress(config) {
	...
	const account = fromCreate2({
		...
		salt: keccak256(concat$1(fromString$1(accountDomain), padLeft$1(config.salt ?? zeroSalt, 32), fromNumber(config.threshold, { size: 1 }), fromNumber(config.owners.length, { size: 1 }), ...config.owners.flatMap((owner) => [owner.owner, fromNumber(owner.weight, { size: 1 })])))
	});
...
function getSignPayload$2(value) {
	...
	return keccak256(concat$1(fromString$1(signatureDomain), from$7(payload), account, fromNumber(config.version ?? 0n, { size: 8 })));
}
/**
...
function assertMultisig(envelope, depth) {
	...
	if (!validate$2(envelope.account)) throw new InvalidMultisigApprovalError({ reason: "multisig account is invalid" });
	...
	for (const inner of envelope.signatures) {
		...
		if (type === "multisig") assertMultisig(inner, depth + 1);
			...
			assert$1(inner);
			if (size$1(serialize$3(inner)) > 2049) throw new InvalidMultisigApprovalError({ reason: `multisig owner signature exceeds ${maxOwnerSignatureBytes} bytes` });
		}
	}
}
/**
...
function deserialize_(value, multisigDepth) {
	const serialized = value.endsWith("0x7777777777777777777777777777777777777777777777777777777777777777".slice(2)) ? slice(value, 0, -size$1(magicBytes)) : value;
	if (size$1(serialized) === 65) {
	...
	if (typeId === serializedMultisigType) {
		...
		if (depth > 2) throw new InvalidSerializedError$1({
			reason: `multisig nesting depth exceeds 2`,
			serialized
		});
		const decoded = toHex(data);
		if (!Array.isArray(decoded) || decoded.length !== 3) throw new InvalidSerializedError$1({
			reason: "invalid multisig wire shape: expected exactly three fields",
			serialized
		});
		const [account, config, signatures] = decoded;
		if (!validate$2(account)) throw new InvalidSerializedError$1({
			reason: "invalid multisig account",
			serialized
		});
		if (!Array.isArray(config)) throw new InvalidSerializedError$1({
			reason: "invalid multisig config",
			serialized
		});
		const [salt, version, threshold, owners] = config;
		if (config.length !== 4 || Array.isArray(salt) || size$1(salt) !== 32 || Array.isArray(version) || size$1(version) > 8 || version !== "0x" && slice(version, 0, 1) === "0x00" || Array.isArray(threshold) || size$1(threshold) > 1 || !Array.isArray(owners) || owners.some((owner) => !Array.isArray(owner) || owner.length !== 2 || owner.some(Array.isArray) || !validate$2(owner[0]) || size$1(owner[1]) > 1)) throw new InvalidSerializedError$1({
			reason: "invalid multisig config",
			serialized
		});
		if (!Array.isArray(signatures) || signatures.some(Array.isArray)) throw new InvalidSerializedError$1({
			reason: "invalid multisig signatures list",
			serialized
		});
		if (signatures.length === 0) throw new InvalidSerializedError$1({
			reason: "multisig signatures cannot be empty",
			serialized
		});
		if (signatures.length > 8) throw new InvalidSerializedError$1({
			reason: `multisig signatures exceed 8`,
			serialized
		});
		for (const signature of signatures) if (size$1(signature) > 2049 && slice(signature, 0, 1) !== "0x05") throw new InvalidSerializedError$1({
...
*/
function fromRpc$5(envelope) {
	if (typeof envelope === "string") {
		const serialized = concat$1(serializedMultisigType, envelope);
		const result = deserialize$3(serialized);
		if (result.type !== "multisig" || !isEqual$1(serialize$3(result), serialized)) throw new InvalidSerializedError$1({
	...
	if (envelope.type === "webAuthn") {
		...
		const webauthnDataSize = size$1(webauthnData);
		let authenticatorData;
		let clientDataJSON;
		for (let split = 37; split < webauthnDataSize; split++) {
			const potentialJson = toString(slice(webauthnData, split));
	...
	if (envelope.type === "keychain" || "userAddress" in envelope && "signature" in envelope) {
		...
		return {
			...
			inner: fromRpc$5(keychain.signature),
...
function toRpc$5(envelope) {
	...
	if (type === "keychain") {
		...
		return {
			type: "keychain",
			userAddress: keychain.userAddress,
			signature: toRpc$5(keychain.inner),
			...keychain.keyId ? { keyId: keychain.keyId } : {},
			...keychain.version ? { version: keychain.version } : {}
		};
	}
...
*/
var InvalidMultisigApprovalError = class extends BaseError$1 {
	constructor({ reason }) {
...
function from$3(authorization, options = {}) {
	if (typeof authorization.chainId === "string") return fromRpc$4(authorization);
	const resolved = {
		...authorization,
...
*/
function fromRpc$4(authorization) {
	const { address, chainId, nonce } = authorization;
	const signature = fromRpc$5(authorization.signature);
	return {
		address,
...
function fromRpcList(authorizationList) {
	return authorizationList.map((x) => fromRpc$4(x));
}
/**
...
function toRpcList(authorizationList) {
	return authorizationList.map((x) => toRpc$4(x));
}
/**
...
function from$2(authorization, options = {}) {
	if ("keyId" in authorization) return fromRpc$3(authorization);
...
function assertWitness(witness) {
	if (size$1(witness) !== 32) throw new InvalidWitnessSizeError(witness);
}
...
var InvalidWitnessSizeError = class extends Error {
	constructor(witness) {
		super(`Witness \`${witness}\` must be exactly 32 bytes (got ${size$1(witness)} bytes).`);
		Object.defineProperty(this, "name", {
			enumerable: true,
			configurable: true,
			writable: true,
...
function fromTupleList(accessList) {
	...
	for (let i = 0; i < accessList.length; i++) {
		const [address, storageKeys] = accessList[i];
		if (address) assert$5(address, { strict: false });
		list.push({
			address,
			storageKeys: storageKeys.map((key) => validate$3(key) ? key : trimLeft(key))
		});
	}
	return list;
}
...
function assert(envelope) {
	...
	if (calls) {
		for (const call of calls) if (call.to) assert$5(call.to, { strict: false });
	}
...
function deserialize$1(serialized) {
	...
	if (feePayerSignatureOrSender !== "0x" && feePayerSignatureOrSender !== void 0) {
		if (feePayerSignatureOrSender === "0x00" || validate$2(feePayerSignatureOrSender)) {
...
async function selectApprovals(options) {
	...
	if (!validate$2(account) || toBigInt(account) === 0n) throw new InvalidApprovalError({ reason: "account is invalid" });
	if (!validate$3(hash)) throw new InvalidApprovalError({ reason: "hash is invalid" });
...
function fromRpc$2(operation) {
	try {
		...
		return from({
			...operation,
			config: fromRpc$6(operation.config)
		});
	} catch (cause) {
		if (cause instanceof InvalidOperationError) throw cause;
		throw new InvalidOperationError({ cause });
...
function assertBase(operation, config) {
	if (!validate$2(operation.account)) throw new InvalidOperationError({ reason: "account is invalid" });
	if (toBigInt(operation.account) === 0n) throw new InvalidOperationError({ reason: "account cannot be zero" });
	if (!validate$3(operation.hash)) throw new InvalidOperationError({ reason: "hash is invalid" });
...
function assertTransaction(operation) {
	...
	if (operation.status === "pending") {
		...
		if (!validate$3(submissionId ?? "")) throw new InvalidOperationError({ reason: "submissionId is invalid" });
		...
		if (!validate$3(transactionHash ?? "")) throw new InvalidOperationError({ reason: "transactionHash is invalid" });
...
var InvalidApprovalError = class extends BaseError$1 {
	/**
...
var InvalidOperationError = class extends BaseError$1 {
	/**
...
function shimKeyData$1(data) {
	const size = size$1(data);
	if (size <= 4) return data;
	return fromNumber(size, { size: 2 });
}
/** Thrown when a native multisig simulation spec is invalid. */
var InvalidSimulationError = class extends BaseError$1 {
	constructor(options) {
...
function fromRpc(transaction, _options = {}) {
	...
	if (transaction.signature) transaction_.signature = fromRpc$5(transaction.signature);
	...
	if (transaction.keyAuthorization) transaction_.keyAuthorization = fromRpc$3(transaction.keyAuthorization);
...
function toRpc(request) {
	...
	if (request.keyAuthorization) request_rpc.keyAuthorization = toRpc$3(request.keyAuthorization);
	if (request.multisigSimulation) request_rpc.multisigSimulation = toRpc$2(request.multisigSimulation);
...
var UnsupportedSourceIdError = class extends BaseError$1 {
...
function fromRlpCursor(cursor, to = "hex", recursiveDepth = 0) {
	...
	return readList(cursor, readLength(cursor, prefix, 192), to, recursiveDepth + 1);
}
...
function fromMultisig(value) {
	...
	const account = {
		...
		async signTransaction(transaction, options) {
			...
			const payload = keccak256$1(await serializer(presign));
...
async function getConfig(client, parameters) {
	...
	return config ? fromRpc$6(config) : null;
}
/**
...
async function signAuthorization(client, parameters) {
	if ("hash" in parameters && parameters.hash || "owner" in parameters && parameters.owner) {
		...
		const request = await (async () => {
			...
			return { keyAuthorization: toRpc$3(from$2({
				account: account.address,
				address: accessKeyAddress,
				chainId: BigInt(chainId),
				signature: from$4({
		...
		const multisig = fromRpc$2(await client.request({
...
function formatMultisig(value) {
	...
	const operation = fromRpc$2(value);
